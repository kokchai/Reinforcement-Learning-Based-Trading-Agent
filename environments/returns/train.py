...
class train_env(gym.Env):
    ...
    def __init__(self, df, ticker, ts=0, model_name=''):
    ...
    def _buy(self, action):
        ...
        crypto_trade_amount = min(abs(action[0]), available_amount)
    ...
    def step(self, action):
        ...
        if self.terminal:
            ...
            df_total_value.to_csv('results/account_value_train_{}.csv'.format(self.model_name))
            df_total_value.columns = ['account_value']
            df_total_value['daily_return'] = df_total_value.pct_change(1)
            sharpe = (252**0.5) * df_total_value['daily_return'].mean() / df_total_value['daily_return'].std()
            
            df_rewards = pd.DataFrame(self.reward_memory)
            df_rewards.to_csv('results/account_rewards_train_{}.csv'.format(self.model_name))
            
            df_total_holding = pd.DataFrame(self.holding_memory)
            df_total_holding.to_csv('results/account_holding_train_{}.csv'.format(self.model_name))
